...
from compiler_ssa.passes.inline_mapper import InlineMapper
from compiler_ssa.ssa_core import SSAValueGenerator
...
class Inline(Pass):
    ...
    def _try_direct_map(self, inst, mapper):
        ...
        from compiler_ssa.ir.instruction import Load
    ...
    def _copy_instruction(self, inst, value_gen, value_map):
        from compiler_ssa.ir.instruction import Load, Store, BinaryOp, Return, Const
        ...
        if inst_type == Load:
            ...
            from compiler_ssa.ir.instruction import BinaryOp
            new_result = value_gen.next()
            if hasattr(inst, 'result'):
                value_map[inst.result] = new_result
            ...
            from compiler_ssa.ir.instruction import Store
            return Store(target=inst.target, value=inst.value)

        elif inst_type == Return:
            from compiler_ssa.ir.instruction import Return
            return Return(value=inst.value)

        elif inst_type == Const:
            from compiler_ssa.ir.instruction import Const
            new_result = value_gen.next()
            if hasattr(inst, 'result'):
                value_map[inst.result] = new_result
